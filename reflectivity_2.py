...
class Strained_Layer(Epitaxial_Layer):
    def __init__(self, structure, thickness_vector, strain_vector):
        self.strain = dict(t=np.diff(thickness_vector), 
                           a=(strain_vector+1)*structure.subs["a"])
        thickness=thickness_vector[len(thickness_vector)-1]
        super(Strained_Layer, self).__init__(structure, thickness)
